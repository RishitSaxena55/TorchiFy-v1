...
class DecoderOnlyTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.dec_layers = nn.ModuleList([SelfAttentionDecoderLayer(d_model, num_heads, d_ff, dropout) for i in
                                         range(self.num_layers)])  # ModuleList of decoder layers
        ...
        self.final_linear = nn.Linear(d_model, self.num_classes)  # Final linear layer
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        # Create source and target embeddings and other layers
        ...
        # Create the encoder normalization layer
        self.encoder_norm = nn.LayerNorm(d_model)  # Encoder normalization

        # Create the decoder normalization layer
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[
        torch.Tensor, torch.Tensor, dict, dict]:
        '''
        ...
        # Apply positional encoding if not skipped
        ...
        # Pass through encoder layers and save attention weights
        running_att = {}
        ...
        # Project to CTC logits
        ctc_logits = self.ctc_head(x_enc)
        ctc_inputs = {
            'log_probs': torch.transpose(ctc_logits, 0, 1),
            'lengths': x_enc_lengths
        }

        # Return the encoded representation, padding mask, running attention weights, and CTC inputs (see docstring)
    ...
    def decode(
        ...
        # Apply the embedding, positional encoding, and dropout
        ...
        # Pass through decoder layers and save attention weights
        running_att = {}
    ...
    def forward(
        ...
        # Return the output sequence, running attention weights, and CTC inputs (see docstring)
    ...
    def score(self, batch_prompts: torch.Tensor, encoder_output: torch.Tensor,
        ...
        # Use decode function with no target lengths (no padding mask for targets)
...
def test_decoder_only(num_layers: int = 12, num_heads: int = 8, d_model: int = 512, d_ff: int = 2048,
                      dropout: float = 0.1, max_len: int = 300, num_classes: int = 1000):
